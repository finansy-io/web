...
export function Drawer(props: DrawerProps) {
	const {isOpen, setIsOpen, title, children, isDismissible, progressSheet} = props;

	return (
		<Root open={isOpen} onOpenChange={setIsOpen} dismissible={isDismissible}>
			<Trigger asChild>
			...
			<Portal>
				...
				>
					<div className='mx-auto flex h-full w-full max-w-md flex-col gap-4 p-2'>
						{progressSheet ? progressSheet : <div className='mx-auto h-[3px] w-10 rounded-full bg-[#BAC3CA]' />}

						{title && <Title className='text-center text-lg font-medium'>{title}</Title>}
